...
x_train - np.rehsape(x_train, (x_train.shape[0], x_train.shape[1], 1))

# Build the model
model = Sequential
model.add(LSTM(units=50, retrun_sequences=True, input_shape=(x_train.shape[1], 1)))
model.add(Dropout(0, 2))
model.add(LSTM(units=50, retrun_sequences=True))
model.add(Dropout(0, 2))
...
plt.plot(actual_prices, color="black", label=f"Acutal {company}")
